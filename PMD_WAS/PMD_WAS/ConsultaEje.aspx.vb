...
Partial Class ConsultaEje
    ...
    Private Sub LlenaDropAño()
        Dim sent2 As New System.Data.SqlClient.SqlCommand("Select Año from Admin_meses where Admon='" & Me.DropAdmon.Text & "'", conx.conectar)
    ...
    Public Sub CargaDrop()
        Dim sent As New System.Data.SqlClient.SqlCommand("Select Cve_admon,Nombr_admon from Cat_Admon", conx.conectar)
    ...
    Public Sub LlenarEjes()
        'LLENAR DROP EJES
        Dim sent3 As New System.Data.SqlClient.SqlCommand("Select IdEje,Desc_eje from Ejes where Admon='" & Me.DropAdmon.SelectedValue & "' order by 2", conx.conectar)
        Dim exe3 As System.Data.SqlClient.SqlDataReader
        exe3 = sent3.ExecuteReader
        Try
            If Not exe3 Is Nothing Then
                GridEje.DataSource = exe3
                GridEje.DataBind()
            End If
            exe3.Close()
        Catch ex As Exception
    ...
    Protected Sub GridEje_OnRowCommadn(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles GridEje.RowCommand
        If e.CommandName = "Buscar" Then
            ...
            'Otra manera de Cambiar color
            'Dim row As GridViewRow = CType(CType(e.CommandSource, LinkButton).NamingContainer, GridViewRow)
            ' CType(row.FindControl("LinkButton1"), LinkButton).ForeColor = Color.Black
            ...
            Dim sent As New System.Data.SqlClient.SqlCommand("Select IdObjetivo,Nombr_obj from Objetivos where IdEje='" & Me.E.Text & "' and Admon='" & Me.DropAdmon.SelectedValue & "'", conx.conectar)
            Dim exe As System.Data.SqlClient.SqlDataReader
            exe = sent.ExecuteReader
            Try
                If Not exe Is Nothing Then
    ...
    Protected Sub GridObj_OnRowCommadn(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles GridObj.RowCommand
        ...
        Dim sent As New System.Data.SqlClient.SqlCommand("Select IdEstrategia,Nombr_estr from Estrategias where IdObjetivo='" & Me.O.Text & "' and IdEje='" & Me.E.Text & "' and Admon='" & Me.DropAdmon.SelectedValue & "'", conx.conectar)
        Dim exe As System.Data.SqlClient.SqlDataReader
        exe = sent.ExecuteReader
        Try
            If Not exe Is Nothing Then
    ...
    Protected Sub GridEstr_OnRowCommadn(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles GridEstr.RowCommand
        ...
        'Dim sent As New System.Data.SqlClient.SqlCommand("Select IdLinea,Nombr_linea from Lineas where IdEstrategia='" & Me.Es.Text & "' and  IdObjetivo='" & Me.O.Text & "' and IdEje='" & Me.E.Text & "' and Admon='" & Me.DropAdmon.SelectedValue & "'", conx.conectar)
        'Dim exe As System.Data.SqlClient.SqlDataReader
        'exe = sent.ExecuteReader
        'Try
        '    If Not exe Is Nothing Then
    ...
    '    Dim sent As New System.Data.SqlClient.SqlCommand("Select IdSublinea,Nombr_Sub from Sublineas where IdLinea='" & Me.L.Text & "' and IdEstrategia='" & Me.Es.Text & "' and  IdObjetivo='" & Me.O.Text & "' and IdEje='" & Me.E.Text & "' and Admon='" & Me.DropAdmon.SelectedValue & "'", conx.conectar)
    '    Dim exe As System.Data.SqlClient.SqlDataReader
    '    exe = sent.ExecuteReader
    ...
    Public Sub Sel() 'Hace El Select con el ID para saber su Unidad de medida y la clasificacion e la linea
        ...
        stry = "select a.ID,a.tipo_de_Linea,b.Nombr_tipo from Concentrado_PMD a inner join Cat_tipo_linea b on a.tipo_de_linea=b.Cve_tipo where a.ID='" & Me.E.Text & "." & Me.O.Text & "." & Me.Es.Text & ""
        'If Me.S.Text = "" Then
        '    stry = stry & "'"
        'Else
        '    stry = stry & "." & Me.S.Text & "' "
        ...
        If exe.Read() = True Then
            ...
            If Me.CveTipoLinea.Text = 1 Then
                'Linea Programada
                stry2 = "select distinct(a.ID),a.Unidad_Medida,a.Clasificacion,isnull(b.Enero,0)as Enero,isnull(b.Febrero,0)as Febrero, " &
                                                             ...
                                                             "left join Informacion_anual b on b.ID=a.ID and b.Cve_admon='" & Me.DropAdmon.Text & "'" &
            ...
            Else
                ...
                'Linea por Solicitud
                stry2 = "select distinct(a.ID),a.Unidad_Medida,a.Clasificacion,isnull(b.Enero,100)as Enero,isnull(b.Febrero,100)as Febrero, " &
    ...
    Public Sub ConsultaDetalleDeLinea()
        ...
        If Me.CveTipoLinea.Text = 1 Then
            ...
            Dim stry4 = "select distinct(a.ID),a.Descr_estrategia,a.Unidad_Medida,c." & Mes & ",isnull(d.Semana1,0)as semana1,isnull(d.Semana2,0)as Semana2,isnull(d.Semana3,0)as Semana3,isnull(d.Semana4,0)as Semana4,isnull(d.Semana5,0)as Semana5,isnull(d.Semana6,0)as Semana6,isnull(d.Acumulado,0) as Acumulado,d.ComentariosP as ComP, " &
                                                                ...
                                                                "from Concentrado_PMD a " &
                                                               "inner join Informacion_Anual c on a.ID=c.ID and c.Cve_admon='" & Me.DropAdmon.Text & "' " &
                                                               "left join Informacion_Planeada d on d.ID=a.ID and d.Mes='" & Mes & "' " &
                                                               "left join Informacion_Real e on e.ID=a.ID and e.Mes='" & Mes & "' " &
        ...
        Else
            Dim stry4 = "select distinct(a.ID),a.Descr_estrategia,a.Unidad_Medida,c." & Mes & ",isnull(d.Semana1,0)as semana1,isnull(d.Semana2,0)as Semana2,isnull(d.Semana3,0)as Semana3,isnull(d.Semana4,0)as Semana4,isnull(d.Semana5,0)as Semana5,isnull(d.Semana6,0)as Semana6,isnull(d.Acumulado,0) as Acumulado,d.ComentariosP as ComP, " &
                                                           ...
                                                           "left join Informacion_Planeada d on d.ID=a.ID and d.Mes='" & Mes & "' " &
                                                           "left join Informacion_Real e on e.ID=a.ID and e.Mes='" & Mes & "' " &
                                                           "left join Informacion_Solicitada f on f.ID=a.ID and f.Mes='" & Mes & "' " &
