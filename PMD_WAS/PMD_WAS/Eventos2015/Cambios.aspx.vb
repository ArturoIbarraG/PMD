...
Partial Class Cambios
    ...
    Private Sub AplicaPermisos()
        ...
        Using data As New DB(con.conectar())
            ...
            ddl2.SelectedValue = dt.Rows(0)("IdSecretaria")

            Session("IDSECRE") = ddl2.SelectedValue
            ...
            If dt.Rows(0)("IdPuesto") <> 1 Then
                ddl3.Enabled = False
            End If
            ddl3.SelectedValue = dt.Rows(0)("IdDireccion")
        End Using
    End Sub
...
#End Region
    ...
    Protected Sub txtfolio_TextChanged(sender As Object, e As System.EventArgs) Handles txtfolio.TextChanged
        ...
        If Me.txtfolio.Text <> "" Then
            ...
            Dim stry As String
            Dim tabla As DataTable
            stry = "select id_secr from [eventos].reg_evento r LEFT JOIN [eventos].map_evento m  On r.folio=m.folio where r.folio =  " & Me.txtfolio.Text & ""
            tabla = conexion.sqlcon(stry)
    ...
    Sub consultaEvento()
        ...
        Try
            ...
            If tabla.Rows.Count < 1 Then
                ...
                'SI YA PASO POR AQUI ES PORQUE SI EXISTE EL REGISTRO MAS NO EXISTE EL MAPA Y ES NECESARIO GUARDARLO
                'ScriptManager.RegisterStartupScript(Page, Page.GetType(), "popup", "alerta_Cambio_Error_FaltaGuardarMapa();", True)
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "Mostrar", "alert('NO HA GUARDADO EL PUNTO EN EL MAPA, FAVOR DE GUARDARLO');", True)
                'Me.lblmsjcambio.Visible = True
                'Me.lblmsjcambio.Text = "*No existe el folio"
                'inicia()
                'deshabilita()
    ...
    Sub consulta()
        ...
        Else
            ...
            If IsDBNull(Me.tabla.Rows(0)("validada")) Then
            Else
                validacion = tabla.Rows(0)("validada")
            End If


            Dim IraAlcalde As Boolean
            If tabla.Rows(0)("alcalde") = False Then
                IraAlcalde = False
            Else
                IraAlcalde = True
            ...
            If validacion = 0 Then
                ...
                consultaEvento()
            Else
                If validacion = 1 And IraAlcalde = False Then
                    'Si esta valdiada y no va alcalde: SI PUEDE REALIZARSE CAMBIOS
                    consultaEvento()
                Else
                    ScriptManager.RegisterStartupScript(Page, Page.GetType(), "popup", "alerta_Cambio_Error_YaValidada();", True)
                    consultaEvento()
                    deshabilita()
                    btnAceptar.Visible = False
                    btnCancelar.Visible = True
    ...
    Sub CargarEvento()
        ...
        stry = "select id_evento,evento from evento"
        tabla = conexion.sqlcon(stry)
    ...
    Function Laborales(FInicio As String, FFin As String) As Integer
        ...
        stry = "select count(fecha) as dias from dias_festivos where fecha between '" & FInicio & "' and '" & FFin & "'"
    ...
    Protected Sub btnAceptar_Click(sender As Object, e As System.EventArgs) Handles btnAceptar.Click
        ...
        If Session("UsuarioAdmin") <> 1234 Then
            ...
            If diashabiles > 4 Then
                ...
                If alcalde <> 0 Then
                    ...
                    stry = "UPDATE [eventos].reg_evento set nombre_evento = '" & Trim(Me.txtEvento.Text) & "',id_secr=" & Trim(Me.ddl2.Text) & ",id_depe=" & Trim(Me.ddl3.Text) & ",id_evento=" & Trim(Me.DropDownList2.Text) & ",descripcion = '" & Trim(Me.txtDescripcion.Text) & "',lugar ='" & Trim(Me.txtLugar.Text) & "', id_col=" & Trim(Me.ddl4.Text) & ",id_calle=" & Trim(Me.ddl5.Text) & ",num_ext='" & Trim(Me.txtext.Text) & "',num_int='" & Trim(Me.txtint.Text) & "',fecha='" & Trim(Me.txtfecha.Text) & "',hr_ini='" & Trim(Me.txthrini.Text) & "',hr_fin='" & Trim(Me.txthrfin.Text) & "',num_benef=" & Trim(Me.txtnumbenef.Text) & ",alcalde =" & alcalde & ",hr_alcalde ='" & Trim(Me.txthralcalde.Text) & "',hr_salida = '" & Trim(Me.txtHrSalidaAlcalde.Text) & "',prensa =" & prensa & ",limpiar = " & limpiar & ",responsable ='" & Me.txtResponsableEvento.Text & "',telefono = " & Me.txtTelefonoResponsable.Text & ",operador = '" & Trim(Me.txtOperadopor.Text) & "',puesto_ope ='" & Trim(Me.txtPuestoOperador.Text) & "',telefono_ope = " & Trim(Me.txtTelefonoOperador.Text) & ",correo_ope = '" & Trim(Me.txtCorreoOperador.Text) & "', validada = 0  WHERE folio =" & Trim(Me.txtfolio.Text) & ""
                Else
                ...
                Else
                    ...
                    stry = "insert into bitacora_movimiento values(" & Session("clave_empl") & ", 'CAMBIOS','Cambios al folio de evento'," & Trim(folio) & " ,getdate())"
                    resultado = conexion.sqlcambios(stry)
        ...
        Else
            ...
            If Me.txtfecha.Text > fecha_captura Then
                ...
                End If
            Else
                'mensaje de que no se puede guardar la fecha porque no se peude capturar el mismo dia tiene que ser uan fecha posterior al dia de hoy
                ' NO GUARDA FECHA
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "popup", "alerta_Cambio_Error_FechaPosterior();", True)
                Exit Sub
            End If
        End If
    ...
    Protected Sub txtOperadopor_TextChanged(sender As Object, e As System.EventArgs) Handles txtOperadopor.TextChanged

    End Sub

    Protected Sub txtCorreoOperador_TextChanged(sender As Object, e As System.EventArgs) Handles txtCorreoOperador.TextChanged
