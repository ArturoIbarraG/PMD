...
Partial Class Requerimientos
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack = True Then
            ...
            Dim complemento = ""
            If Request.QueryString("folio") IsNot Nothing Then
                complemento = "?folio=" + Request.QueryString("folio")
            End If

            If Request.QueryString("readOnly") IsNot Nothing Then
                complemento = complemento + "&readOnly=1"
            End If

            If dependencia = 276 And secretaria = 313 Then
                Response.Redirect("~/Eventos2015/RequerimientosComunicacion.aspx" + complemento)
            End If

            If puesto < 4 Then
                Response.Redirect("~/Eventos2015/RequerimientosDirector.aspx" + complemento)
            Else
                Response.Redirect("~/Eventos2015/RequerimientosEventos.aspx" + complemento)
            End If
        ...
        Try
            ...
            If dt IsNot Nothing Then
                For Each dr In dt.Rows
                    ...
                    Dim costo As Decimal = dr("COSTO")
                    Dim iva As Decimal = dr("IVA")
                    costoTotal = costoTotal + (costo * cantidad)
                    Try
                        ivaTotal = ivaTotal + (costo * (1 + (iva / 100)))
                    Catch ex As Exception

                    End Try
                    totalTotal = totalTotal + (total * cantidad)
                Next
            End If

            lblSubtotalActual.Text = String.Format("{0:c2}", costoTotal)
            lblIvaActual.Text = String.Format("{0:c2}", ivaTotal)
            lblPresupuestoActual.Text = String.Format("{0:c2}", totalTotal)
        Catch ex As Exception
        ...
        If Me.txtFolio.Text <> "" Then
            ...
            Else
                ...
                Dim con As New conexion
                    ...
                    stry = "select hr_insta,fecha_insta,presidium,aforo FROM [eventos].reg_requerimiento where folio_evento =" & Me.txtFolio.Text & "  GROUP BY hr_insta,fecha_insta,presidium,id_ceremonia,aforo"
                    tabla = conexion.sqlcon(stry)
                    If tabla.Rows.Count < 1 Then
                    ...
                    Else
                        ...
                        If IsDBNull(tabla.Rows(0)("aforo")) Then
                            Me.txtAforo.Text = ""
                        Else

                            Me.txtAforo.Text = tabla.Rows(0)("aforo")
                        End If
    ...
    Private Sub CargaInformacionImprevistos(folio As Integer)
        ...
        If imprevistos Is Nothing Or imprevistos.Count = 0 Then
            ...
            Using data As New DB(con.Conectar())
                ...
                Dim dt = data.ObtieneDatos("ObtieneCatalogoImprevistos", params).Tables(0)
    ...
    Protected Sub btnAgregarImprevistos_Click(sender As Object, e As EventArgs)
        ...
        If GridView1.Rows.Count > 0 Then
            For i As Integer = 0 To GridView1.Rows.Count - 1
                ...
                If HttpUtility.HtmlDecode(GridView1.Rows(i).Cells(2).Text) = 26 Then
                    Continue For
                End If
                dr("TIPO") = HttpUtility.HtmlDecode(GridView1.Rows(i).Cells(1).Text)
                dr("ID") = HttpUtility.HtmlDecode(GridView1.Rows(i).Cells(2).Text)
        ...
        dr("ID") = HttpUtility.HtmlDecode(Me.lblidreq.Text)
        dr("DESCRIPCIÓN") = drpReq.SelectedItem.Text
        dr("CANTIDAD") = txtMontoChequeImprevistos.Text
        Dim requerimientosLista As String = ""
        For Each row In imprevistos.Where(Function(x) x.Asignado = True)
            requerimientosLista += row.Nombre & ", "
        Next
