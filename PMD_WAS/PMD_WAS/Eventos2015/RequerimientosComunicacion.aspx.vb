...
Imports conexion
Partial Class RequerimientosComunicacion
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack = True Then
            'VALIDA EL PUESTO DE EL USUARIO LOGUEADO Y DEPENDIENDO LO MANDA A SU PAGINA DE REQUERIMIENTOS
            Dim puesto = Session("Puesto")
            Dim secretaria = Session("id_secr")
            Dim dependencia = Session("clave_depe")
            If secretaria <> 313 And dependencia <> 276 And puesto < 4 Then
                Response.Redirect("~/Eventos2015/RequerimientosDirector.aspx")
            ElseIf puesto = 4 Then
                Response.Redirect("~/Eventos2015/RequerimientosEventos.aspx")
            End If
    ...
    Public Class RequerimientoItem
        ...
        Public Property ClaveGastos As Integer
        Public Property Presupuesto As Decimal
    End Class

    Public Class PrespupuestoDetalle
        Public Property Contrato As String
        Public Property Presupuesto As Decimal
        ...
        Try
            ...
            If lista.Count > 0 Then
                For Each dr In lista
                    ...
                    costoTotal = costoTotal + total '(costo * cantidad)
            ...
            Dim desglocePresupuesto As New List(Of PrespupuestoDetalle)()
            Using data As New DB(con.Conectar())

                For Each row In lista
                    Dim contrato = row.NumContrato + " - " + row.Contrato
                    If desglocePresupuesto.Exists(Function(x) x.Contrato = contrato) Then
                        Dim desg = desglocePresupuesto.FirstOrDefault(Function(x) x.Contrato = contrato)
                        desg.Comprometido = desg.Comprometido + row.Total
                        desg.Disponible = desg.Presupuesto - desg.Comprometido

                    Else
                        Dim pres = New PrespupuestoDetalle()
                        pres.Comprometido = data.ObtieneDatos("ObtienePresupuestoComprometido", New SqlParameter() {New SqlParameter("@folio", txtFolio.Text),
                                          New SqlParameter("@id", row.Id),
                                          New SqlParameter("@tipo", row.Tipo)}).Tables(0).Rows(0)("Comprometido")
                        pres.Contrato = contrato
                        pres.Comprometido = row.Total
                        pres.Presupuesto = row.Presupuesto
                        pres.Disponible = pres.Presupuesto - pres.Comprometido
                        pres.Proveedor = row.Proveedor
                        desglocePresupuesto.Add(pres)
                    End If
                Next

            End Using

            rptDetallePresupuesto.DataSource = desglocePresupuesto
            rptDetallePresupuesto.DataBind()
            Session("dtDesglocePresupuestoE") = desglocePresupuesto
        Catch ex As Exception
    ...
    Protected Sub txtFolio_TextChanged(sender As Object, e As System.EventArgs) Handles txtFolio.TextChanged
        ...
        If Me.txtFolio.Text <> "" Then
            ...
            Else
                ...
                Try
                    If dt.Rows(0)("validadoDireccion") = True And dt.Rows(0)("validadoEnlaceAdmin") = True And dt.Rows(0)("validadoComunicacion") = False Then
                        'EL EVENTO ESTA LISTO PARA VALIDAR POR ENLACE ADMINISTRATIVO
                        btnValidarEvento.Enabled = True
                    ElseIf dt.Rows(0)("validadoComunicacion") = True Then
                        btnValidarEvento.Enabled = False
                    ElseIf dt.Rows(0)("validadoDireccion") = False Or dt.Rows(0)("validadoEnlaceAdmin") = False Then
                        'EL EVENTO AUN NO HA SIDO VALIDADO POR EL DIRECTOR
                        ScriptManager.RegisterStartupScript(UpdatePanel1, UpdatePanel1.GetType(), "mensaje", "alert('El evento aún no ha sido validado por el Director, no se puede revisar.');", True)
                        btnValidarEvento.Enabled = False
    ...
    Private Sub CargaRequerimientosDirector()
        ...
        Using data As New DB(con.Conectar())
            ...
            For Each row In dt.Rows
                ...
                rq.IVA = row("IVA")
                rq.Total = row("TOTAL")
                rq.Proveedor = row("PROVEEDOR")
                rq.NumContrato = row("CODIGO_CONTRATO")
                rq.Contrato = row("NOMBRE_CONTRATO")
                rq.ClaveGastos = row("CLAVE_GASTOS")
                rq.Presupuesto = row("PRESUPUESTO")
                Try
        ...
        grdRequerimientos.DataSource = requerimientosLista.OrderBy(Function(x) x.Tipo).ThenBy(Function(x) x.Contrato).ToList()
    ...
    Private Sub CargaCatalogoRequerimientos()
        Using data As New DB(con.Conectar())
            ...
            Dim catalogoRequerimiento = data.ObtieneDatos("CargaRequerimientosDirector", params).Tables(0)
            Session("CatalogoRequerimientosComunicacion") = catalogoRequerimiento
            ddlRequerimientoDirector.DataSource = catalogoRequerimiento
        ...
        'CARGA CONTRATOS
        CargaCatalogoContrato()

        'CARGA PROVEDORES
        CargaCatalogoProveedores()
    End Sub
    ...
    Protected Sub btnAgregarImprevistos_Click(sender As Object, e As EventArgs)
        ...
        grdRequerimientos.DataSource = requerimientos.OrderBy(Function(x) x.Tipo).ThenBy(Function(x) x.Contrato).ToList()
        grdRequerimientos.DataBind()
        recalculaPresupuesto()
    ...
    Protected Sub btnAgregarRequerimiento_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            r.Presupuesto = dt.Rows(0)("presupuesto")
            r.IVA = dt.Rows(0)("iva")
            ...
            If tipo = 1 Then
                r.TipoNombre = "Contrato"
            ElseIf tipo = 2 Then
                r.TipoNombre = "Requisición"
            ElseIf tipo = 3 Then
                r.TipoNombre = "Orden de Servicio"
            End If

            r.Contrato = dt.Rows(0)("nombre_contrato")
            r.Proveedor = dt.Rows(0)("proveedor")
            r.NumContrato = dt.Rows(0)("codigo_contrato")
            r.ClaveGastos = dt.Rows(0)("clave_gastos")
            requerimientos.Add(r)
...
#End Region
            ...
            'Limpia variables
            Session("RequerimientosEvento") = New List(Of RequerimientoItem)()
            Session("dtDesglocePresupuestoE") = New List(Of PrespupuestoDetalle)()
            recalculaPresupuesto()
        End If
    ...
    Private Sub CargaCatalogoContrato()
        Dim dt As DataTable = Session("CatalogoRequerimientosComunicacion")
        'Dim rows = (From row In dt.AsEnumerable()
        '            Select row.Field(Of String)("User")).Distinct()

        Dim contratos = dt.DefaultView.ToTable(True, "nombre_contrato")
        Dim c = contratos.NewRow()
        'c("codigo_contrato") = "-1"
        c("nombre_contrato") = "Todos"
        contratos.Rows.InsertAt(c, 0)
        ddlContrato.DataValueField = "nombre_contrato"
        ddlContrato.DataTextField = "nombre_contrato"
        ddlContrato.DataSource = contratos
        ddlContrato.DataBind()
    End Sub

    Private Sub CargaCatalogoProveedores()
        Dim dt As DataTable = Session("CatalogoRequerimientosComunicacion")

        If ddlContrato.SelectedItem.Text <> "Todos" Then
            dt = dt.Select(String.Format("nombre_contrato='{0}'", ddlContrato.SelectedItem.Text)).AsEnumerable().CopyToDataTable()
        End If

        Dim proveedores = dt.DefaultView.ToTable(True, "proveedor")
        Dim r = proveedores.NewRow()
        r("proveedor") = "Todos"
        proveedores.Rows.InsertAt(r, 0)
        ddlProveedor.DataValueField = "proveedor"
    ...
    Protected Sub ddlContrato_SelectedIndexChanged(sender As Object, e As EventArgs)
        CargaCatalogoProveedores()
        RecargaRequerimientos()
    End Sub
    Protected Sub ddlProveedor_SelectedIndexChanged(sender As Object, e As EventArgs)
        RecargaRequerimientos()
    End Sub

    Private Sub RecargaRequerimientos()
        Dim dt As DataTable = Session("CatalogoRequerimientosComunicacion")
        ...
        If ddlProveedor.SelectedItem.Text <> "Todos" Then
            dt = dt.Select(String.Format("proveedor='{0}'", ddlProveedor.SelectedItem.Text)).AsEnumerable().CopyToDataTable()
        End If

        ddlRequerimientoDirector.DataSource = dt
        ddlRequerimientoDirector.DataTextField = "requerimiento"
        ddlRequerimientoDirector.DataValueField = "id"
        ddlRequerimientoDirector.DataBind()
    End Sub
    Protected Sub btnValidarEvento_Click(sender As Object, e As EventArgs)
        Dim helper As New IntelipolisEngine.Eventos.EventoHelper()
        helper.AgregaValidacionEvento(txtFolio.Text, Session("clave_empl"), 3)
